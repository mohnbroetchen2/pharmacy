...
class Job(WeeklyJob):
    ...
    def execute(self):
        ...
        from .models import Pharmacy, Person, Submission, DrugClass, Company, Order, Profile
        ...
        try:
            ...
            if recipients:
                ...
                for o in orderlist:
                    if o.expiry_date > today + timedelta(days=7) and o.expiry_date < today:
                        orderlist = orderlist.exclude(pk=o.pk)
                    if len(orderlist)>0:
                        for recipient in recipients:
                            message = render_to_string('expiration_mail.html',{'delta':delta, 'orderlist':orderlist, 'first_name':recipient.user.first_name})
